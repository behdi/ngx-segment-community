...
import {
  ...
  SEGMENT_CONFIG,
  SegmentConfig,
} from './ngx-segment-analytics.config';
...
export class SegmentService {
  ...
  protected static readonly _segmentInstance: AnalyticsBrowser =
    new AnalyticsBrowser();

  /**
   * @param userConfig Segment configuration
   */
  constructor(@Inject(SEGMENT_CONFIG) userConfig: SegmentConfig) {
    this._config = { ...DEFAULT_CONFIG, ...userConfig };

    if (
      this._config.loadOnInitialization &&
      (typeof this._config.apiKey === 'undefined' || this._config.apiKey === '')
    ) {
      console.error(
    ...
    if (
      this._config.loadOnInitialization &&
      !SegmentService._segmentInstance.instance?.initialized
    ) {
   ...
   */
  public load(
    settingsOrApiKey: AnalyticsBrowserSettings | string,
    options: InitOptions = {},
  ): void {
  ...
  public async page(
    ...
    await SegmentService._segmentInstance.page(
      category,
      name,
      properties,
      options,
   ...
   */
  public trackForm(
    forms: HTMLFormElement | HTMLFormElement[],
    event: string | Function,
    properties?: any | Function,
  ): void {
  ...
  public addDestinationMiddleware(
    ...
    SegmentService._segmentInstance.addDestinationMiddleware(
      integration,
      ...middlewares,
    );
  }
